...
def chunkandconv(data, size_img, n_chunks, overlap):
    """
        ...
        for x in range(n_chunks[0]):
            ...
            for y in range(n_chunks[1]):
                ...
                for z in range(n_chunks[2]):
                    ...
                    if ((data[0][j] >= xstart - overlap and data[0][j] < (xstart + size_chunk[0] + overlap)) and
                        (data[1][j] >= ystart - overlap and data[1][j] < (ystart + size_chunk[1] + overlap)) and
                        (data[2][j] >= zstart - overlap and data[2][j] < (zstart + size_chunk[2] + overlap))):
    ...
    N = torch.tensor(range(size_chunk[0]))
    zi = N.expand(size_chunk[0], size_chunk[1], size_chunk[2])
    xi = zi.transpose(0, 2)
    yi = zi.transpose(1, 2)
    ...
    for x in range(n_chunks[0]):
        ...
        for y in range(n_chunks[1]):
            ...
            for z in range(n_chunks[2]):
                ...
                intensityspot = torch.zeros((size_chunk[0], size_chunk[1], size_chunk[2]))
                ...
                img[xstart:xstart + size_chunk[0], ystart:ystart + size_chunk[1], zstart:zstart + size_chunk[2]] = intensityspot
...
# what's our current best value for the optimal image size/chunk size ratio?
chunkfact_opt = 19
# how many chunks are we splitting the data into along each dimension? (calculates using chunkfact_opt)
n_chunks = np.array([size_img[0]/chunkfact_opt, size_img[0]/chunkfact_opt, size_img[0]/chunkfact_opt]).astype(int)
...
for i in range(nimg):
    ...
    mtubs = chunkandconv(data, size_img, n_chunks, overlap)
