...
nn.Linear() performs a linear transform from one layer to the next (applies weights and biases)
...
nn.LogSoftmax()
          ...
          then takes the log() of this result, as it reduces the instances of over & underflow (i.e. too many digits)
"""
def ANN(layers):
...
padding='same' adds zero-padding to the outside of the layer before convolution such that the size of the layer is preserved
padding='valid' for some reason this is the term used to mean 'no padding' so that the layers shrink on each convolution
