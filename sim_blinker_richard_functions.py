...
def v_img(structA, rads=np.pi/10, edgSpace=1/7, scaleFact=10):
    """
    creates two converging straight lines
    """
    ...
    # Obtain the coefficients for the 1st order lines that join the points into
    # a V shape & define the lines in terms of those coefficients:
...
def NoisePic(pixelsY, pixelsX, NumFrames, cameraBase=400, cameraSTD=25, backCount=25):
    ...
    # EMCCD base level in ADC
    cameraBase = 2*200*1.0
    # per pixel standard deviation in base level
    cameraSTD = 0.5*50.0
    NumFrames = round(NumFrames)
    # background light level in photons per frame per pixel
    # backCount = 25.0*1.0/10
