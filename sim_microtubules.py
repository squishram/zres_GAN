...
3. Uses a pooling-style approach to convert the random walk coordinates into array 'signal' values.
   This is done by convolving each coordinate with a gaussian to simulate the PSF/ position uncertainty. 
...
def random_walk(t, size, max_step=0.25, sharpest=2*np.pi, reinitialise=True):
    ...
    for q in range(1, t):
        ...
        if reinitialise==True and (((x[q] > (size[0] + 1)) or (x[q] < -1))
...
# PSF specs: 
# What is the mean intensity (AU) and its uncertainty (as a proportion of mean intensity)?
intensity_mean = 5
int_unc = 0.2
# What is the mean sigma in xy (voxels?) and the sigma uncertainty (as a proportion of mean intensity)?
sigma_xy_mean = 0.5
sig_unc = 0.2
...
# how many chunks per dimension? (this will be cubed to get the total number of chunks)
n_chunks = 2
...
# step size in pixels each iteration (make it <0.5 if you want continuous microtubules):
max_step = 1
...
for i in range(nimg):
    ...
    # broadcast intensity & sigma values into arrays with distributed random values so each emitter gets its own ones:
    intensity = np.array([intensity_mean*(1 + + r.uniform(-int_unc, int_unc)) for i in range(len(x))])
    sigma_xy = np.array([sigma_xy_mean*(1 + r.uniform(-sig_unc, sig_unc)) for i in range(len(x))])
    sigma_z = np.array([sigma_z_mean*(1 + r.uniform(-sig_unc, sig_unc)) for i in range(len(x))])
