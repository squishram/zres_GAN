...
path_lores = os.path.join(path_data, Path("microtubules/lores_undersampledz"))
path_hires = os.path.join(path_data, Path("microtubules/hires_undersampledz"))
...
n_imgs = 300
# file name root:
filename = "mtubs_sim_undersampledz"
...
chunk_overlap = np.array(
    [
        int(chunk_overlap * sigma_xy_mean),
        int(chunk_overlap * sigma_xy_mean),
        int(chunk_overlap * sigma_z_mean),
    ]
)
...
for i in range(n_imgs):
    ...
    for idx, sigma_z_mean in enumerate(sigma_tuple):
        # broadcast intensity & sigma values into arrays
        intensity = np.array(
            [
        ...
        mtubs = image_of_gaussians(data, size_img, n_chunks, chunk_overlap)
...
print(
    f"Done! To make {2 * n_imgs} {img_bit}-bit images with {n_chunks[0]} chunks/axis took {time2 - time1} seconds"
)
