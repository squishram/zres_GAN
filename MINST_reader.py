...
# How many in a batch?
batch = 64
# Give me 2 numbers in a tuple whose product is <batch (this is the grid we will display that part of the batch in, for inspection)
disp_count = (6, 10)
# How many epochs (i.e. complete cycles through the full training dataset during gradient descent)?
epochs = 15
# learning rate?
learnRate = 0.003
# momentum?
...
# .next() retrieves the next item in dataiter (first batch the first time, second batch second time)
...
# The layers list contains the number of nodes in each layer - add the first and last layers to the beginning and end respectively
...
# nn.NLLLoss() is The 'negative log likelihood loss' - it is a type of loss function
...
#          it's often better than the sum of squared residuals/ mean squared error, as it computes a much larger loss at outputs that are very wrong
...
# nn.torch.optim.SGD() is the method by which the size of that step is taken - stochastic gradient descent
# SGD just means you use a random subset of the full dataset when calculating each step size, instead of the whole thing
# This makes each step less accurate, but less computationally expensive, and therefore faster
#          >model.parameters() are the weights and biases obtained from the model (defined above)
...
for e in range(epochs):
    ...
    for images, labels in trainloader:
        ...
        # The output is what the network thinks, the labels are the actual correct answer. 
        # Apply the cost function to these two, and you get the loss!
        loss = cost(output, labels)
        
        # backward() computes the gradient, so we know how big the step should be
