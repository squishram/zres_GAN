...
def load_cube(zres: Number, prefix: str = "") -> torch.Tensor:
    ...
    for i in sorted(
        Path("tmp/").glob("{}zres-{}-slice-*.fits".format(prefix, int(zres)))
    ):
        print(i)
...
def conv_1D_z_axis(
    ...
    """Assuming data is a cube [z,y,x], then perform a 1D convolution along the z axis"""
    assert len(kernel.size()) == 1, "Kernel is not 1D"
    assert len(data.size()) == 3, "Data is not a cuboid"
    assert (
        len(kernel) % 2 == 1
    ), "Kernel must be odd sized (symmetric and centred on the middle element"
...
def xyz_projections(data: torch.Tensor) -> torch.Tensor:
    ...
    assert data.size(0) == data.size(1) and data.size(0) == data.size(
        2
    ), "Data is not a cube"
...
def fourier_anisotropy_loss_pnorm(
    ...
    assert (
        len(filter_ft) == math.floor(data.size(0) / 2) + 1
...
sigma_extra = math.sqrt(
    fwhm_to_sigma(z_res_real / nm_per_pix) ** 2
...
hpf_ft = highpass_gaussian_kernel_fourier(
    fwhm_to_sigma(z_res_real / nm_per_pix), N=data_normal.size(2)
).to(device)
sampling_window = blackman_harris_window(128).to(device)
z_kernel = gaussian_kernel(sigma_extra, 3.0).to(device)

# starting point for the reconstruction is the square root of the reconstruction (data) that we want
...
# there are only two for loops because we are going to plot some things every 50 iterations
for _ in range(1000):
    for i in range(50):
        print("--------------------------", i, "------------------------")
...
# fta = complex_abs(torch.rfft(x_only, signal_ndim=1, normalized=False, onesided=True))
# plt.subplot(1, 2, 2)
